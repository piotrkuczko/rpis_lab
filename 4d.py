...
def MonteCarlo(formula, formula_variables_count, samples_count):
    clauses_count = len(formula)
    chosen_clauses = np.random.randint(clauses_count, size=samples_count)
    result = 0

    for chosen_clause in chosen_clauses:
        value = np.random.choice([True, False], variables_count + 1)

        for var in formula[chosen_clause]:
            value[abs(var)] = var > 0
        ...
        for clause in formula:
            ...
            for var in clause:
                if value[abs(var)] == ((-var) > 0):
                    satisfying = False
                    break

            if satisfying:
                value_prob += (1 / 2) ** (formula_variables_count - len(clause))

        result += clauses_count / value_prob
    return result


samples_count = 1000

total_satisfying_assignments = MonteCarlo(phi, variables_count, samples_count)
print(round(total_satisfying_assignments / samples_count))
